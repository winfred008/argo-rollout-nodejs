pipeline {
    ...
    environment {
    DOCKERHUB_CREDENTIALS = credentials('winfred008-dockerhub')
    APP_NAME = "winfred008/bluegreen-rollout-mar"
    }
    stages { 
        stage('SCM Checkout') {
            steps{
           git branch: 'main', url: 'https://github.com/winfred008/Argo-rollout-nodejs.git'
            }
        }
        // run sonarqube test
        stage('Run Sonarqube') {
